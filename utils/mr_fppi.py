...
def lamr(miss_rate, fppi, num_of_samples=9):
    ...
    for i in range(interpolated.shape[0]):
        if interpolated[i] != 0:
            new_interpolated.append(interpolated[i])
    new_log_interpolated = np.log(new_interpolated)
    # new_log_interpolated = []

    # new_log_interpolated = [log_interpolated[i] for i in range(len(log_interpolated))] if log_interpolated[i] != -inf
    avg = sum(new_log_interpolated) / len(new_log_interpolated)
