...
def intersection(a, b):
    ...
    """
    intersection_top_left_x = max(a['bbox'][0], b.x_top_left)
    intersection_top_left_y = max(a['bbox'][1], b.y_top_left)
    intersection_bottom_right_x = min(a['bbox'][0] + a['bbox'][2],  b.x_top_left + b.width)
    intersection_bottom_right_y = min(a['bbox'][1] + a['bbox'][3], b.y_top_left + b.height)
...
def iou(a, b):
    ...
    union_area = a['bbox'][2] * a['bbox'][3] + b.width * b.height - intersection_area
...
def match_detections(detection_results, ground_truth, overlap_threshold, overlap_fn=iou):
    ...
    for image_id, annotaitons in ground_truth.items():
...
def match_detection_to_annotations(detection, annotations, overlap_threshold, overlap_fn):
    ...
    for i, annotation in enumerate(annotations):
        if annotation["category_id"] != detection.class_label:
            continue
